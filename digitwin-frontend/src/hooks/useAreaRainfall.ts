// Hook to calculate area rainfall using EXACT same logic as DataCards
...
export function useAreaRainfall(planningAreaId: string): number {
  ...
  const rainfall = useMemo(() => {
    ...
    // Calculate area center coordinates (same as DataCards areaCenter)
    ...
    // IDW interpolation function (EXACT copy from DataCards)
    const interpolateValue = (stations: any[], readings: any[], getValue: (r: any) => number): number | null => {
      let weightedValue = 0;
      let totalWeight = 0;
    ...
    // Step 3: No local station, use IDW interpolation (EXACT same as DataCards)
    else {
      const avgRainfall = interpolateValue(stations, readings, (r) => r.value);
      console.log(`🌧️ Rainfall for ${planningAreaId}: ${avgRainfall?.toFixed(2) ?? 0.00} mm (IDW interpolation)`);
